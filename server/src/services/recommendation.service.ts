...
const CGPA_WEIGHT = 0.25;
const PACKAGE_WEIGHT = 0.15;
const CGPA_MARGIN_RANGE = 3;
...
export const recommendationService = {
  ...
  async recommendDrivesForStudent(userId: string, limit = 10): Promise<DriveRecommendation[]> {
    ...
    const scored = candidates.map((drive) => {
      ...
      const skillScore = jaccardSimilarity(studentSkillNames, driveSkillNames);
      const cgpaMarginScore = clamp01((profile.cgpa - drive.minCgpa) / CGPA_MARGIN_RANGE);
      const packageScore = maxPackage === 0 ? 0 : drive.packageLPA / maxPackage;

      const score =
        SKILL_WEIGHT * skillScore + CGPA_WEIGHT * cgpaMarginScore + PACKAGE_WEIGHT * packageScore;

      return { drive, score: Math.round(score * 100) / 100, matchedSkills };
    });
